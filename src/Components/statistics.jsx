...
import {statistics} from '../util/const'
    ...
    <div className={`${styles.flexStart} flex-row flex-wrap sm:mb-20 mb-6 `}>
        {
            statistics.map((stats) =>(
                <div key={stats?.id} className={`flex-1 flex justify-start items-center flex-row m-3 `}>
                    ...
                    <p className='font-montserrat font-normal xs:text-[20px] text-[16px ] xs:leading-[26px] leading-[22px] text-gradient uppercase  ml-3'>{stats?.title}</p>
                </div>
            ) )
        }
    </div>
